...
if [ "$1" = 'cassandra-reaper' ]; then
    ...
    gosu reaper /usr/local/bin/configure-persistence.sh
    gosu reaper /usr/local/bin/configure-webui-authentication.sh
    gosu reaper /usr/local/bin/configure-metrics.sh
    exec gosu reaper /usr/bin/java \
                    ${JAVA_OPTS} \
